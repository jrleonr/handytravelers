...
<form role="form" ref="form" method="POST" action="{{ route('invitation.addMessage') }}">
    {{ csrf_field() }}
    <input type="hidden" name="invitationId" value="{{ $invitation->uuid}}">

    <p class="control">
        <textarea name="body" class="textarea" rows="5" id="message" placeholder="{{ $invitation->getTextByUserRole($type) }}"></textarea>
    </p>
    ...
    <p class="control">
        ...
        @elseif($invitation->isPending() && $invitation->userRole == 'guest')
        <div class="control is-grouped">
            <p class="control">
